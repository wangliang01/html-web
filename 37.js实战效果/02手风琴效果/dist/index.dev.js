...
for (var i = 0; i < labels.length; i++) {
  ...
  label.addEventListener('click', function () {
    var subMenu = this.nextElementSibling; // 找到有opened的元素，先进行关闭

    var openedSubMenu = document.querySelector('.sub-menu[data-status="opened"]');
...
function open(subMenu) {
  if (!subMenu.dataset.status || subMenu.dataset.status === 'closed') {
    ...
    animate({
      ...
      duration: 300,
      onProcess: function onProcess(value) {
        subMenu.style.height = value + 'px'; // 同时设置status 
